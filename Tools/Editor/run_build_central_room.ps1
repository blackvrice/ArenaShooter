...
$scriptFile = Join-Path $projectRoot "Tools\Editor\build_central_room.py"
...
if ($openEditor) {
    throw "ArenaShooter is open in Unreal Editor. Close the editor before rebuilding the central room."
}

try {
    if ($InspectOnly) {
        $env:ARENA_CENTRAL_ROOM_MODE = "inspect"
    } else {
        Remove-Item Env:ARENA_CENTRAL_ROOM_MODE -ErrorAction SilentlyContinue
    }
    ...
    if ($commandletExitCode -ne 0) {
        ...
        $expectedMarker = if ($InspectOnly) { "CENTRAL_ROOM_INSPECT=" } else { "Central room rebuilt" }
        ...
        if (-not $pythonSucceeded -or $pythonFailed) {
            throw "Central room commandlet failed with exit code $commandletExitCode."
        }
        Write-Warning "Unreal returned exit code $commandletExitCode because of editor/source-control warnings, but the central room script completed successfully."
    }
} finally {
    Remove-Item Env:ARENA_CENTRAL_ROOM_MODE -ErrorAction SilentlyContinue
}
